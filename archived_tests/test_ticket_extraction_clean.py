...
def test_ticket_extraction():
    ...
    try:
        ...
        if successful_extractions:
            print(f"\n[SUCCESS] COMPLETE EXTRACTIONS (Both user and manager found):")
            for result in successful_extractions:
                print(f"   Ticket #{result['ticket_number']}: {result['user_email']} -> {result['manager_email']}")
        ...
        if partial_extractions:
            print(f"\n[WARNING] PARTIAL EXTRACTIONS (User found, manager missing):")
            for result in partial_extractions:
                print(f"   Ticket #{result['ticket_number']}: {result['user_email']} (no manager)")
        
        # Show failed extractions
        failed_extractions = [r for r in extraction_results if not r['has_user']]
        if failed_extractions:
            print(f"\n[ERROR] FAILED EXTRACTIONS (No user email found):")
            for result in failed_extractions:
                print(f"   Ticket #{result['ticket_number']}: No user email found")
